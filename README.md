...
- **Latest change:** every Reference section now carries an emoji marker and the sections are **sorted alphabetically by title** for faster scanning. Just before: a **Discover 🟠 Cashback Categories** table (quarterly, synced, auto-cleared each new year); **arithmetic in `$` fields** (`+ - * /` and parentheses via a safe recursive-descent evaluator, applied to Full price / Amount paid / split amount); and the first four Reference header emojis.
