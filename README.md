...
- **Drag-and-drop ordering**, collapsible month/week grouping, and dual sort modes (by date logged vs. by planned date). In *By date logged*, weeks stay chronological but each week internally surfaces ☑️ executed planning entries first, then ⏳ open planning entries, then regular expenses (drag order still wins).
...
- **Latest change:** the **By date logged** view now applies a within-week priority — ☑️ executed planning entries first, then ⏳ open planning entries, then regular expenses (bounties count as planning; manual drag order still wins; weeks themselves stay chronological) — making executed planned expenses easy to spot. Just before: open bounties gained a **month reset** (stale planned date cleared + auto-🚩 on the first open of a new month), and Enter now evaluates `$`-field arithmetic in place.
